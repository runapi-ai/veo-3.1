...
- For a one-off artifact or result, use the registered `veo-3-1` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi veo-3-1 --help
runapi veo-3-1 <operation> --help
curl --fail --location https://runapi.ai/docs/api/veo-3-1/<operation>.md --output contract.md
```
...
```shell
runapi veo-3-1 <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service veo-3-1 --action <operation> > result.json
```
